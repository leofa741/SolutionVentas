    ...
    public partial class FrmCategorias : Form
    {
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Opocion= MessageBox.Show("Deseas eliminar el registro?","Sistema de verntas",MessageBoxButtons.OKCancel,MessageBoxIcon.Question);
        ...
        private void btnActivar_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult Opocion;
                Opocion = MessageBox.Show("Deseas activar el registro?", "Sistema de verntas", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (Opocion == DialogResult.OK)
                {
                    int codigo;
                    string Rpta = "";
                    foreach (DataGridViewRow item in DgvListado.Rows)
                    {
                        if (Convert.ToBoolean(item.Cells[0].Value))
                        {
                            ...
                            if (Rpta.Equals("Ok"))
                            {
                                this.MensajeOk("Se activi correctamente" + Convert.ToString(item.Cells[2].Value));
                            }
                            else
                            {
                                this.MensajeError(Rpta);
        ...
        private void btnDesactivar_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult Opocion;
                Opocion = MessageBox.Show("Deseas desactivar el registro?", "Sistema de verntas", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (Opocion == DialogResult.OK)
                {
                    int codigo;
                    string Rpta = "";
                    foreach (DataGridViewRow item in DgvListado.Rows)
                    {
                        if (Convert.ToBoolean(item.Cells[0].Value))
                        {
                            ...
                            if (Rpta.Equals("Ok"))
                            {
                                this.MensajeOk("Se desactivo correctamente" + Convert.ToString(item.Cells[2].Value));
                            }
                            else
                            {
                                this.MensajeError(Rpta);
